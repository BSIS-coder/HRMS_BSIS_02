...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['action'])) {
        switch ($_POST['action']) {
            ...
            case 'edit_skill':
                try {
                    $stmt = $pdo->prepare("UPDATE skill_matrix SET skill_name=?, description=?, category=? WHERE skill_id=?");
                    $stmt->execute([
                        $_POST['skill_name'],
                        $_POST['description'] ?: null,
                        $_POST['category'],
            ...
            case 'edit_employee_skill':
                try {
                    $stmt = $pdo->prepare("UPDATE employee_skills SET employee_id=?, skill_id=?, proficiency_level=?, assessed_date=?, notes=? WHERE employee_skill_id=?");
                    $stmt->execute([
                        $_POST['employee_id'],
                        $_POST['skill_id'],
                        $_POST['proficiency_level'],
                    ...
                    $message = "Employee skill assessment updated successfully!";
                    $messageType = "success";
                } catch (PDOException $e) {
                    $message = "Error updating employee skill: " . $e->getMessage();
                    $messageType = "error";
                }
                break;
            ...
            case 'add_assessment':
                try {
                    ...
                    $stmt->execute([
                        ...
                        $_POST['skills_gap'],
                        $_POST['recommended_trainings'] ?: null,
                        $_POST['priority'],
                        $_POST['status']
                    ]);
            ...
            case 'edit_assessment':
                try {
                    $stmt = $pdo->prepare("UPDATE training_needs_assessment SET employee_id=?, assessment_date=?, skills_gap=?, recommended_trainings=?, priority=?, status=? WHERE assessment_id=?");
                    $stmt->execute([
                        $_POST['employee_id'],
                        $_POST['assessment_date'],
                        $_POST['skills_gap'],
                            ...
                            <table class="table" id="employeeSkillTable">
                                ...
                                <tbody id="employeeSkillTableBody">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <span class="proficiency-badge proficiency-<?php echo str_replace(' ', '-', strtolower($es['proficiency_level'])); ?>">
    ...
    <script>
        ...
        function openModal(mode, id = null) {
            if (mode === 'add_skill') {
                ...
                document.getElementById('skill_action').value = 'add_skill';
                document.getElementById('skillForm').reset();
                document.getElementById('skill_id').value = '';
                document.getElementById('skillModal').style.display = 'block';
            } else if (mode === 'edit_skill' && id) {
                document.getElementById('skillModalTitle').textContent = 'Edit Skill';
                document.getElementById('skill_action').value = 'edit_skill';
                document.getElementById('skill_id').value = id;
                populateSkillForm(id);
                document.getElementById('skillModal').style.display = 'block';
            } else if (mode === 'add_employee_skill') {
                document.getElementById('employeeSkillModalTitle').textContent = 'Add Employee Skill Assessment';
                document.getElementById('employee_skill_action').value = 'add_employee_skill';
                document.getElementById('employeeSkillForm').reset();
                ...
                document.getElementById('employeeSkillModal').style.display = 'block';
            } else if (mode === 'edit_employee_skill' && id) {
                document.getElementById('employeeSkillModalTitle').textContent = 'Edit Employee Skill Assessment';
                document.getElementById('employee_skill_action').value = 'edit_employee_skill';
                document.getElementById('employee_skill_id').value = id;
                populateEmployeeSkillForm(id);
                document.getElementById('employeeSkillModal').style.display = 'block';
            } else if (mode === 'add_assessment') {
                document.getElementById('assessmentModalTitle').textContent = 'Add Training Needs Assessment';
                document.getElementById('assessment_action').value = 'add_assessment';
                ...
                document.getElementById('assessmentModalTitle').textContent = 'Edit Training Needs Assessment';
                document.getElementById('assessment_action').value = 'edit_assessment';
        ...
        function populateEmployeeSkillForm(employeeSkillId) {
            const es = employeeSkillsData.find(x => x.employee_skill_id == employeeSkillId);
            if (es) {
                document.getElementById('employee_id').value = es.employee_id || '';
                document.getElementById('skill_id').value = es.skill_id || '';
                document.getElementById('proficiency_level').value = es.proficiency_level || '';
                document.getElementById('assessed_date').value = (es.assessed_date || '').toString().substr(0, 10);
                document.getElementById('notes').value = es.notes || '';
            }
        }

        function populateAssessmentForm(assessmentId) {
            const a = assessmentsData.find(x => x.assessment_id == assessmentId);
            if (a) {
                document.getElementById('assessment_employee_id').value = a.employee_id || '';
                document.getElementById('assessment_date').value = (a.assessment_date || '').toString().substr(0, 10);
                document.getElementById('skills_gap').value = a.skills_gap || '';
                document.getElementById('recommended_trainings').value = a.recommended_trainings || '';
                document.getElementById('priority').value = a.priority || '';
                document.getElementById('status').value = a.status || '';
            }
        }

        function editSkill(skillId) { openModal('edit_skill', skillId); }
        function editEmployeeSkill(employeeSkillId) { openModal('edit_employee_skill', employeeSkillId); }
        function editAssessment(assessmentId) { openModal('edit_assessment', assessmentId); }

        function deleteSkill(skillId) {
            if (confirm('Are you sure you want to delete this skill? This action cannot be undone.')) {
                const form = document.createElement('form');
                form.method = 'POST';
                form.innerHTML = '<input type="hidden" name="action" value="delete_skill"><input type="hidden" name="skill_id" value="' + skillId + '">';
        ...
        function deleteEmployeeSkill(employeeSkillId) {
            if (confirm('Are you sure you want to delete this employee skill assessment? This action cannot be undone.')) {
                const form = document.createElement('form');
                form.method = 'POST';
                form.innerHTML = '<input type="hidden" name="action" value="delete_employee_skill"><input type="hidden" name="employee_skill_id" value="' + employeeSkillId + '">';
                document.body.appendChild(form);
                form.submit();
            }
        }
        ...
        function deleteAssessment(assessmentId) {
            if (confirm('Are you sure you want to delete this training needs assessment? This action cannot be undone.')) {
                const form = document.createElement('form');
                form.method = 'POST';
                form.innerHTML = '<input type="hidden" name="action" value="delete_assessment"><input type="hidden" name="assessment_id" value="' + assessmentId + '">';
                document.body.appendChild(form);
                form.submit();
            }
        }
        ...
        window.onclick = function(event) {
            if (event.target === document.getElementById('skillModal')) closeModal('skill');
            else if (event.target === document.getElementById('employeeSkillModal')) closeModal('employeeSkill');
            else if (event.target === document.getElementById('assessmentModal')) closeModal('assessment');
        };
