...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['action'])) {
        switch ($_POST['action']) {
            case 'add_session':
                try {
                    $cost = isset($_POST['cost_per_participant']) && $_POST['cost_per_participant'] !== '' ? $_POST['cost_per_participant'] : null;
                        ...
                        <table class="table" id="sessionTable">
                            ...
                            <tbody id="sessionTableBody">
                                ...
                                <tr>
                                    ...
                                    <td><?php echo htmlspecialchars(trim(($session['first_name'] ?? '') . ' ' . ($session['last_name'] ?? '')) ?: 'N/A'); ?></td>
                                    ...
                                    <td>
                                        <span class="status-badge status-<?php echo str_replace(' ', '-', strtolower($session['status'])); ?>">
    ...
    <script>
        ...
        function populateEditForm(sessionId) {
            ...
            if (session) {
                ...
                document.getElementById('start_date').value = (session.start_date || '').toString().substr(0, 10);
                document.getElementById('end_date').value = (session.end_date || '').toString().substr(0, 10);
                ...
                document.getElementById('status').value = session.status || 'Scheduled';
            }
        }
